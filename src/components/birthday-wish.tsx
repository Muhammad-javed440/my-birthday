...
export default function BirthdayWish() {
  ...
  // Constants
  const totalCandles: number = 9 // Total number of candles
  const totalBalloons: number = 11 // Total number of balloons
  ...
  return (
    // Main container
    <div className="min-h-screen bg-yellow-200 flex items-center justify-center p-1">
      ...
      >
        {/* Birthday card */}
        <Card className="mx-auto overflow-hidden bg-yellow-100 transition-all duration-300 ease-in-out hover:shadow-xl border-2 border-black">
          {/* Card header with birthday message */}
          <CardHeader className="text-center">
            <CardTitle className="text-4xl font-bold text-red-900">Happy 44th Birthday!</CardTitle>
            <CardDescription className="text-2xl font-semibold text-[#331200]">Dear Muhammad Javed</CardDescription>
            <p className="text-lg text-green-700">September 28th</p>
          </CardHeader>
          {/* Card content with candles and balloons */}
          <CardContent className="space-y-6 text-center">
            {/* Candles section */}
            <div>
              <h3 className="text-lg font-semibold text-yellow-800 mb-2">Light the candles:</h3>
                  ...
                  <AnimatePresence key={index}>
                    ...
                    {(celebrating && index <= candlesLit) || (!celebrating && index < candlesLit) ? (
                      ...
                      >
                        {/* Lit candle */}
                        <FaBirthdayCake
                          className={`w-10 h-10 transition-colors duration-500 ease-in-out cursor-pointer hover:scale-110`}
                      ...
                      // Unlit candle
                      <FaBirthdayCake
                        className={`w-10 h-10 text-gray-300 transition-colors duration-500 ease-in-out cursor-pointer hover:scale-110`}
                        onClick={() => lightCandle(index)}
                      />
                    )}
                  </AnimatePresence>
            ...
            {/* Balloons section */}
            <div>
              <h3 className="text-lg font-semibold text-green-900 mb-2">Pop the balloons:</h3>
              <div className="flex justify-center space-x-2">
          ...
          <CardFooter className="flex justify-center">
            <Button 
              className="bg-red-800 text-white hover:bg-red-600 transition-all duration-300"
              onClick={celebrate}
